...
def average_perceptron(feature_matrix, labels, T):
    ...
    """
    current_theta = sum_theta = np.zeros((feature_matrix.shape[1]))
    current_theta_0 = sum_theta_0 = 0
    for t in range(T):
        for i in get_order(feature_matrix.shape[0]):
            # Your code here
             (current_theta, current_theta_0) = perceptron_single_step_update(feature_matrix[i],labels[i],current_theta,current_theta_0)
...
def pegasos(feature_matrix, labels, T, L):
    ...
    upds = 1 #this counts how many updates have been performed
    
    for t in range(T):
        for i in get_order(feature_matrix.shape[0]):
            #eta = 1/sqrt(upds)
             (current_theta, current_theta_0) = pegasos_single_step_update(feature_matrix[i],labels[i],L,(1/np.sqrt(upds)),current_theta,current_theta_0)
             upds = upds + 1
...
def classify(feature_matrix, theta, theta_0):
    ...
    classified = np.sign((preds-epsilon))
...
def classifier_accuracy(
    ...
    # Train the classifier as per whichever function is given by the argument
    (theta, theta_0) = classifier(train_feature_matrix,train_labels,**kwargs)
    
    # Classify the training set and calculate the accuracy
    predicted_train_labels = classify(train_feature_matrix,theta, theta_0)
    accuracy_train = accuracy(predicted_train_labels,train_labels)
    
    # Classify the validation set and calculate the accuracy
    predicted_val_labels = classify(val_feature_matrix,theta, theta_0)
    accuracy_val = accuracy(predicted_val_labels,val_labels)
    ...
    for i, text in enumerate(reviews):
        ...
        for word in word_list:
            if word in dictionary:
                # feature_matrix[i, dictionary[word]] = 1 #this is the binary feature
                feature_matrix[i, dictionary[word]] = feature_matrix[i, dictionary[word]] + 1 #this is the non-binary feature
